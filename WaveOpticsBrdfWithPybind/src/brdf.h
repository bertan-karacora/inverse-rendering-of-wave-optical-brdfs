...
class WaveBrdfAccel {
    public:
        ...
        ComplexfD queryIntegralDiff(const Query &query, const GaborBasisDiff &gaborBasis, int layer, int xIndex, int yIndex);
        Float queryBrdf(const Query &query, const GaborBasis &gaborBasis);
        FloatD queryBrdfDiff(const Query &query, const GaborBasisDiff &gaborBasis);
        BrdfImage genBrdfImage(const Query &query, const GaborBasis &gaborBasis);
        BrdfImage genBrdfImageDiff(const Query &query, const HeightfieldDiff &hf, BrdfImage ref);
